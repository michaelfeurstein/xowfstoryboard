...
namespace eval ::xowfstoryboard {
  ...
  ad_proc schedule_storyboard_validation {object} {
	# pseudo code
	#
	# as long as [object get property first_time_working] is 0
	# schedule_job:check_storyboard in the next 10 seconds
	# as soon as check_storyboard returns storyboard_status 1
	# update [object set property first_time_working] to the elapsed time
	# and don't schedule the next job
	#

	set current_time [dt_systime -format %H:%M:%S]
	set start_clock_seconds [clock scan $current_time -format %H:%M:%S]

	set end_clock_seconds [expr {$start_clock_seconds + 60}]
    set end_time [clock format $end_clock_seconds -format %H:%M:%S]

	if {![info exists party_id]} {
      set party_id [::xo::cc user_id]
    }
	ns_log notice "current_time:$current_time  start_clock_seconds:$start_clock_seconds end_clock_seconds:$end_clock_seconds end_time:$end_time party_id:$party_id first_time_working:[$object get_property -name first_time_working]"

	# Option 1 --> creates a revision but does not fire next_state / only activate / editor still empty
	#$object call_action -action save


	#$object schedule_job -time [clock format $end_clock_seconds -format "%Y-%m-%d %H:%M:%S"] -party_id $party_id [list call_action -action validate_storyboard]
	#set id [ns_schedule_proc -thread 10 {::xowfstoryboard::validate_storyboard}]
  }

  ad_proc validate_storyboard {} {
	ns_log notice "hello from validate"
  }
